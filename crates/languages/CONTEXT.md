...
- **[Simple](./camel-language-simple/CONTEXT.md)** — lightweight expression and predicate language for header, body, property, and exception access. It parses source once into an AST and reuses it. `${lang:expr}` delegation resolves the target Language during each evaluation.
