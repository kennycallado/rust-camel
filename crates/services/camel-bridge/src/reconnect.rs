...
/// new bridge process announces its port and a mTLS channel is established.
///
/// # Contract
...
pub trait BridgeReconnectHandler: Send + Sync + std::fmt::Debug {
    /// Called after a new bridge process starts and its channel is connected.
    ///
    /// `channel` is the mTLS tonic channel to the new bridge.
