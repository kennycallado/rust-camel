...
async fn pre_start_pending_start_registers_and_invokes() {
    ...
    let mut service = FunctionRuntimeService::with_fake_provider(FunctionConfig::default(), Arc::clone(&provider));
    ...
    let _ = inv.invoke(&d1.id, &Exchange::new(Message::new("x"))).await.unwrap();
    let calls = provider.calls.lock().expect("calls").clone();
    assert!(calls.iter().any(|c| matches!(c, FakeCall::Register(_, id) if *id == d1.id)));
    assert!(calls.iter().any(|c| matches!(c, FakeCall::Register(_, id) if *id == d2.id)));
}

#[tokio::test]
async fn pre_start_register_failure_rolls_back_and_keeps_pending() {
    let provider = Arc::new(FakeProvider::new(FakeProviderConfig { fail_on_register: 1, ..Default::default() }));
    let mut service = FunctionRuntimeService::with_fake_provider(FunctionConfig::default(), Arc::clone(&provider));
    let inv = service.invoker();
...
async fn post_start_register_is_immediate_and_invocable() {
    ...
    inv.invoke(&d.id, &Exchange::new(Message::new("x"))).await.unwrap();
}

#[tokio::test]
async fn boot_timeout_errors_and_no_leak() {
    let provider = Arc::new(FakeProvider::new(FakeProviderConfig { fail_on_health: true, ..Default::default() }));
    let cfg = FunctionConfig { health_interval: std::time::Duration::from_millis(20), boot_timeout: std::time::Duration::from_millis(80), ..Default::default() };
...
async fn stop_shutdowns_and_invoke_unavailable() {
    ...
    let err = inv.invoke(&d.id, &Exchange::new(Message::new("x"))).await.unwrap_err();
...
async fn health_task_transitions_to_unhealthy() {
    let provider = Arc::new(FakeProvider::new(FakeProviderConfig::default()));
    let cfg = FunctionConfig { health_interval: std::time::Duration::from_millis(20), boot_timeout: std::time::Duration::from_millis(200), ..Default::default() };
    let mut service = FunctionRuntimeService::with_fake_provider(cfg, Arc::clone(&provider));
    let inv = service.invoker();
    ...
    assert!(matches!(state, camel_function::RunnerState::Unhealthy { .. } | camel_function::RunnerState::Failed { .. }));
}
