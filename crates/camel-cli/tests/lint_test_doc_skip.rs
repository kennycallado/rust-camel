//! End-to-end integration test for the `camel lint` reserved-document skip.
//!
//! `*.test.yaml` files are `camel test` documents and `*.job.yaml` files
//! are `camel job` documents — neither is a route definition.
//! `camel lint <demo.test.yaml>` must exit 0, print exactly one info line
//! naming the file and the reason (reserved document) to stdout, and
//! emit no diagnostics to stderr. The skip uses
//! `camel_dsl::discovery::is_reserved_document`, the same predicate route
//! discovery applies (task 1.3 of `test-placement-contract`; job family
//! added by `job-ux-reshape`).
...
fn cli_lint_test_doc_prints_single_info_line() {
    ...
    assert!(
        lines[0].contains("reserved document"),
...
/// A `*.job.yaml` sibling gets the same reserved-document skip as the
/// test family: exit 0, one info line, no diagnostics (`job-ux-reshape`).
#[test]
fn cli_lint_job_doc_also_skipped() {
    let dir = tempfile::tempdir().expect("tempdir");
    let doc = dir.path().join("demo.job.yaml");
    std::fs::write(&doc, "routeFiles: [x]\nexecute: {}\n").expect("write demo.job.yaml");
    ...
    assert!(
        output.status.success(),
        "linting a job document must exit 0; stdout:\n{stdout}\nstderr:\n{stderr}"
    );
    ...
    assert!(
        lines[0].contains("demo.job.yaml"),
        "info line must name the file; got: {}",
        lines[0]
    );
    assert!(
...
/// endpoints. The document is skipped as a reserved document (info line,
...
fn test_doc_with_mock_intercepts_skipped_no_rmock() {
    ...
    assert!(
        stdout.contains("reserved document"),
        "info line must state the skip reason; got:\n{stdout}"
    );
    assert!(
        !stderr.contains("R-MOCK-IN-PRODUCTION"),
