...
LEAN tier route loading resolves `${env:}` placeholders against the document `env:` map first, then each token's inline `:-default` (rc-l7m7t; ADR-0069 section 13.1). Route files load through `camel_dsl::load_from_file_with_env` with the document-env lookup — the same per-file parser `camel run` uses, including the 16 MiB cap and path-annotated errors; inline `routes:` are interpolated through the same lookup, then parsed through `camel_dsl::parse_yaml`. In both sources a substituted leaf keeps STRING typing (rc-93wct rev 2): a whole-scalar `:-default` token at a string position resolves to its default, a whole-scalar placeholder on an integer- or boolean-typed field fails the document (boot parity, also when the document `env:` map supplies the value), and an unset variable without a default fails the document naming the variable; the process environment is never consulted, so runs stay hermetic.
