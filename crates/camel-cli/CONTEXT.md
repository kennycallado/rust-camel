...
LEAN tier route loading resolves `${env:}` placeholders default-only (ADR-0069 section 13.1). Route files load through `camel_dsl::load_from_file` — the same per-file parser `camel run` uses, including the 16 MiB cap and path-annotated errors; inline `routes:` are interpolated, then parsed through `camel_dsl::parse_yaml`. In both sources a substituted leaf keeps STRING typing (rc-93wct rev 2): a whole-scalar `:-default` token at a string position resolves to its default, a whole-scalar placeholder on an integer- or boolean-typed field fails the document (boot parity), and an unset variable without a default fails the document naming the variable; the process environment is never consulted, so runs stay hermetic.

The unit-tier document identifier fields interpolate `${env:NAME:-default}` the same default-only way, through the camel-dsl scanner, for name-match parity with the interpolated route sources (rc-4hexo; the doc-env layering track rc-l7m7t proposes the surrounding mechanism, ADR-0069 section 4). These fields are the `repositories` and `beans` map keys, the `intercepts` source keys and action targets, the `mock:` references in `expects` keys and `sequence` entries, and the `inputs[].to` values. Interpolation runs right after deserialization and before the other document-validation guards, so the scheme, blank-name, and built-in-name checks see the resolved value. Assertion data never interpolates, and the path fields `routeFiles` and `routeFilesFromRoot` never interpolate: input `body` and `headers`, `expectReply` blocks, matcher contents, bean `methods` and `config` values, repository stub targets, and `settle` all keep the literal text. An identifier placeholder without a default fails document validation at exit 2 naming the variable and the field, mirroring the route-side wording; the ambient environment is never consulted, so the run stays hermetic.
