//! Application layer of the datasource vertical slice.
//!
//! This module owns the use-case orchestration: pool resolution, factory
//! matching, lazy pool creation with health-check wiring, and factory
//! registration. The `DatasourceHealthCheck` private helper implements
//! `AsyncHealthCheck` against the `PoolFactory::check` port for the catalog's
//! optional `HealthCheckRegistry` integration. `scheme_hint` is a small
//! pure helper that extracts the URL scheme for safe error messages without
//! leaking credentials.
//!
//! The trait delegation that exposes these use-cases through the
//! `camel_api::datasource::DatasourceCatalog` port lives in the sibling
//! ring-three module.
...
use super::domain::{CacheKey, RuntimeDatasourceCatalog};
            ...
            let handle = cell
                .get_or_try_init(|| async {
                    ...
                    if let Some(registry) = self.health_registry() {
                        // Uses pseudo-route IDs "datasource:<name>" for health registration.
                        // mark_route_started activates the check in check_all() reports.
                        // This is intentional: datasources are global services, not per-route,
                        // but the HealthCheckRegistry API is route-scoped.
