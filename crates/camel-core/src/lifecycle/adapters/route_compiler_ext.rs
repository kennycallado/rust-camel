...
// Security-plan compilation lives in `route_security`; re-exported here
// so existing imports and the compile_route_security test filter keep
// working without churn (split per the 1k-line rule, bd rc-ivro).
pub use super::route_security::compile_route_security_plan;
pub(crate) use super::route_security::transport_from_uri;
...
pub(crate) fn build_eh_config_pipeline(
    ...
    Ok(if let Some(config) = eh_config {
        ...
        // Strict mode (Task 2.9): the layer is carrier-only — it evaluates the
        // policy against the kernel-minted typed carrier and never
        // authenticates on its own.
        let security = security_policy.map(|sp| {
            BoxProcessor::new(
                SecurityPolicyLayer::new(sp.policy, transport).layer(IdentityProcessor),
            )
        });
        ...
        if let Some(sp_config) = security_policy {
            let sp_layer = SecurityPolicyLayer::new(sp_config.policy, transport);
...
impl RouteCompilerExt<'_> {
    ...
    fn compile_route_impl(
        ...
        let transport = transport_from_uri(def.from_uri());
