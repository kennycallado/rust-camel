...
impl RouteControllerHandle {
    ...
    pub async fn insert_prepared_route(&self, prepared: PreparedRoute) -> Result<(), CamelError> {
        let (reply_tx, reply_rx) = oneshot::channel();
        self.tx
    ...
    let handle = tokio::spawn(async move {
        ...
        while let Some(cmd) = rx.recv().await {
            match cmd {
                ...
                RouteControllerCommand::AddRouteDefinitionWithGeneration {
                    definition,
                    generation,
                    reply,
                } => {
                    let _ = reply.send(
                        controller
                            .add_route_with_generation(definition, generation)
                            .await,
                    );
                }
