//! Application layer of the health_registry vertical slice.
//!
//! This module owns the use-case orchestration: `check_all` runs all registered
//! health checks (live or forced) with timeouts, catches panics, and aggregates
//! into a `HealthReport`. The `CheckTask` enum is the internal representation
//! of a probe task — a use-case-internal type, not a domain entity.
//!
//! The state-mutating inherent methods on `HealthCheckRegistry` live in
//! `domain` (they are entity operations, not orchestration). This module
//! extends the same struct with the orchestration method.

use std::sync::Arc;

use camel_api::{CheckResult, HealthReport, HealthStatus, ServiceHealth};
use chrono::Utc;
