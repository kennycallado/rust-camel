...
mod executor {
    ...
    impl PoolFactory for StubPoolFactory {
        fn create<'a>(&'a self, config: &'a DatasourceConfig) -> CreatePoolFuture<'a> {
            Box::pin(async move {
                // AnyPool connect fails without the compiled-in
                // drivers registered (camel-sql pool_factory.rs
                // precedent). max_connections(1) keeps all statements
                // on one connection: a private :memory: DB per
                // connection would otherwise split CREATE/INSERT
                // state (camel-sql test precedent).
    ...
    /// Seeds `stmts` through the real prepare executor; a seed
    /// failure is a test-harness defect, never the subject under
    /// test, so it panics with the executor's own error text.
