...
use camel_api::{
    Body, BoxProcessor, BoxProcessorExt, Exchange, IdentityProcessor, Message, Value,
};
...
fn bench_expression(c: &mut Criterion) {
    ...
    group.bench_function("simple_language_eval", |b| {
        b.to_async(&rt).iter(|| {
            ...
            ex.input
                .set_header("x", Value::String("hello".into()));
            async move { pipeline.oneshot(ex).await.unwrap() }
        })
    });
