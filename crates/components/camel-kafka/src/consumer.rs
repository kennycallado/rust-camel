...
impl RdConsumerContext for ReadyContext {
    fn post_rebalance(&self, rebalance: &Rebalance<'_>) {
        if matches!(rebalance, Rebalance::Assign(_)) {
            // Partitions were assigned — persist one readiness signal.
            // `notify_one()` stores a permit if no task is currently awaiting,
            // which prevents lost notifications in test code that calls
            // `ready_signal()` before `start()` but awaits shortly after.
