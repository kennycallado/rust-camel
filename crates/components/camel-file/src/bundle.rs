...
use crate::{FileComponent, FileGlobalConfig};

pub struct FileBundle {
    config: FileGlobalConfig,
}

impl ComponentBundle for FileBundle {
    fn config_key() -> &'static str {
        "file"
    }

    fn from_toml(value: toml::Value) -> Result<Self, CamelError> {
        let config: FileGlobalConfig = value
            .try_into()
    ...
    fn register_all(self, ctx: &mut dyn ComponentRegistrar) {
        ctx.register_component_dyn(Arc::new(FileComponent::with_config(self.config)));
    }
}
...
mod tests {
    ...
    #[test]
    fn file_bundle_from_toml_empty_uses_defaults() {
        let value: toml::Value = toml::from_str("").unwrap();
        let result = FileBundle::from_toml(value);
        assert!(
            result.is_ok(),
    ...
    #[test]
    fn file_bundle_registers_expected_schemes() {
        let bundle = FileBundle::from_toml(toml::Value::Table(toml::map::Map::new())).unwrap();
        let mut registrar = TestRegistrar { schemes: vec![] };

        bundle.register_all(&mut registrar);

        assert_eq!(registrar.schemes, vec!["file"]);
    }

    #[test]
    fn file_bundle_from_toml_returns_error_on_invalid_config() {
        ...
        let result = FileBundle::from_toml(toml::Value::Table(table));
