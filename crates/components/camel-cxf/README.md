...
```
cxf://http://host:port/path?wsdl=path/to/service.wsdl&service={namespace}ServiceName&port={namespace}PortName&operation=operationName
```
...
|-----------|---------|-------------|
| `wsdl` | _(required)_ | Path to WSDL file |
| `service` | _(required)_ | Service name with namespace in `{ns}Name` format |
| `port` | _(required)_ | Port name with namespace in `{ns}Name` format |
| `operation` | _(none)_ | SOAP operation name (can also be set via header `CamelCxfOperation`) |

> `address` is the URI path portion after `cxf://` — for producers this is the target endpoint URL, for consumers this is the bind address. Service-level config (security, address overrides) lives in `Camel.toml`.
...
Multiple services are supported — add as many `[[components.cxf.services]]` entries as needed. Each unique WSDL/service/port combination gets its own bridge process.
...
The component manages Java bridge processes (GraalVM native-image binaries) internally:

1. On first use, downloads the bridge binary from GitHub releases (SHA256-verified, cached at `~/.cache/rust-camel/cxf-bridge/`).
2. Spawns one bridge process per unique WSDL/service/port combination.
...
**Producer flow:** Exchange body (raw XML) → gRPC call → Java CXF bridge → SOAP HTTP call → response XML back to exchange body.

**Consumer flow:** Vert.x HTTP server receives SOAP requests → gRPC stream → Rust route processes → response XML → bridge wraps in SOAP envelope → HTTP response.
