    ...
    let result: serde_json::Value = match cmd {
        RedisCommand::Publish => {
            let channel = require_str_header(exchange, "CamelRedis.Channel")
                .map(|s| s.to_string())
                .map_err(|_| CamelError::ProcessorError(
                    "Missing required header: CamelRedis.Channel".into()
                ))?;
            
            let message = exchange.input.body.as_text()
                .ok_or_else(|| CamelError::ProcessorError(
                    "Message body must be text for PUBLISH".into()
                ))?;
