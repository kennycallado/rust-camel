...
use super::{get_str_header, get_f64_header, get_i64_header, get_bool_header, 
            get_str_vec_header, require_key, require_value};
...
pub async fn dispatch(
    ...
    let result: serde_json::Value = match cmd {
        RedisCommand::Zadd => {
            ...
            let n: i64 = conn.zadd(&key, member.to_string(), score).await
        ...
        RedisCommand::Zrem => {
            let key = require_key(exchange)?;
            let member = require_value(exchange)?;
            let n: i64 = conn.zrem(&key, member.to_string()).await
        ...
        RedisCommand::Zrange => {
            ...
            let with_scores = get_bool_header(exchange, "CamelRedis.WithScore").unwrap_or(false);
            if with_scores {
                let vals: Vec<(String, f64)> = conn.zrange_withscores(&key, start, end).await
                ...
                let vals: Vec<String> = conn.zrange(&key, start, end).await
                    .map_err(|e| CamelError::ProcessorError(format!("Redis ZRANGE failed: {e}")))?;
        ...
        RedisCommand::Zrevrange => {
            ...
            let with_scores = get_bool_header(exchange, "CamelRedis.WithScore").unwrap_or(false);
            if with_scores {
                let vals: Vec<(String, f64)> = conn.zrevrange_withscores(&key, start, end).await
                    .map_err(|e| CamelError::ProcessorError(format!("Redis ZREVRANGE failed: {e}")))?;
                serde_json::json!(vals.into_iter()
                    .map(|(member, score)| serde_json::json!({"member": member, "score": score}))
                    .collect::<Vec<_>>())
            } else {
                let vals: Vec<String> = conn.zrevrange(&key, start, end).await
                    .map_err(|e| CamelError::ProcessorError(format!("Redis ZREVRANGE failed: {e}")))?;
                serde_json::json!(vals)
            }
        }
        RedisCommand::Zrank => {
            let key = require_key(exchange)?;
            let member = require_value(exchange)?;
            let rank: Option<i64> = conn.zrank(&key, member.to_string()).await
                .map_err(|e| CamelError::ProcessorError(format!("Redis ZRANK failed: {e}")))?;
            rank.map(|r| serde_json::json!(r)).unwrap_or(serde_json::Value::Null)
        }
        RedisCommand::Zrevrank => {
            let key = require_key(exchange)?;
            let member = require_value(exchange)?;
            let rank: Option<i64> = conn.zrevrank(&key, member.to_string()).await
        ...
        RedisCommand::Zscore => {
            let key = require_key(exchange)?;
            let member = require_value(exchange)?;
            let score: Option<f64> = conn.zscore(&key, member.to_string()).await
                .map_err(|e| CamelError::ProcessorError(format!("Redis ZSCORE failed: {e}")))?;
            score.map(|s| serde_json::json!(s)).unwrap_or(serde_json::Value::Null)
        }
        RedisCommand::Zcard => {
            let key = require_key(exchange)?;
        ...
        RedisCommand::Zincrby => {
            ...
            let member = require_value(exchange)?;
            let new_score: f64 = conn.zincr(&key, member.to_string(), increment).await
        ...
        RedisCommand::Zcount => {
            ...
            let n: i64 = conn.zcount(&key, min, max).await
        ...
        RedisCommand::Zrangebyscore => {
            ...
            let with_scores = get_bool_header(exchange, "CamelRedis.WithScore").unwrap_or(false);
            if with_scores {
                let vals: Vec<(String, f64)> = conn.zrangebyscore_withscores(&key, min, max).await
                    .map_err(|e| CamelError::ProcessorError(format!("Redis ZRANGEBYSCORE failed: {e}")))?;
                serde_json::json!(vals.into_iter()
                    .map(|(member, score)| serde_json::json!({"member": member, "score": score}))
                    .collect::<Vec<_>>())
            } else {
                let vals: Vec<String> = conn.zrangebyscore(&key, min, max).await
                    .map_err(|e| CamelError::ProcessorError(format!("Redis ZRANGEBYSCORE failed: {e}")))?;
                serde_json::json!(vals)
            }
        }
        RedisCommand::Zrevrangebyscore => {
            ...
            let with_scores = get_bool_header(exchange, "CamelRedis.WithScore").unwrap_or(false);
            if with_scores {
                let vals: Vec<(String, f64)> = conn.zrevrangebyscore_withscores(&key, max, min).await
                    .map_err(|e| CamelError::ProcessorError(format!("Redis ZREVRANGEBYSCORE failed: {e}")))?;
                serde_json::json!(vals.into_iter()
                    .map(|(member, score)| serde_json::json!({"member": member, "score": score}))
                    .collect::<Vec<_>>())
            } else {
                let vals: Vec<String> = conn.zrevrangebyscore(&key, max, min).await
                    .map_err(|e| CamelError::ProcessorError(format!("Redis ZREVRANGEBYSCORE failed: {e}")))?;
                serde_json::json!(vals)
            }
        }
        RedisCommand::Zremrangebyrank => {
            ...
            let start = get_i64_header(exchange, "CamelRedis.Start").unwrap_or(0) as isize;
            let end = get_i64_header(exchange, "CamelRedis.End").unwrap_or(-1) as isize;
            let n: usize = conn.zremrangebyrank(&key, start, end).await
                .map_err(|e| CamelError::ProcessorError(format!("Redis ZREMRANGEBYRANK failed: {e}")))?;
        ...
        RedisCommand::Zremrangebyscore => {
            ...
            let n: i64 = redis::cmd("ZREMRANGEBYSCORE")
                ...
                .map_err(|e| CamelError::ProcessorError(format!("Redis ZREMRANGEBYSCORE failed: {e}")))?;
            serde_json::json!(n)
        }
        RedisCommand::Zunionstore => {
            ...
            let n: i64 = conn.zunionstore(dest, &keys).await
                .map_err(|e| CamelError::ProcessorError(format!("Redis ZUNIONSTORE failed: {e}")))?;
            serde_json::json!(n)
        }
        RedisCommand::Zinterstore => {
            ...
            let keys = get_str_vec_header(exchange, "CamelRedis.Keys")
                .unwrap_or_else(|| vec![require_key(exchange).unwrap_or_default()]);
            let n: i64 = conn.zinterstore(dest, &keys).await
                .map_err(|e| CamelError::ProcessorError(format!("Redis ZINTERSTORE failed: {e}")))?;
            serde_json::json!(n)
        }
        _ => return Err(CamelError::ProcessorError("Not a sorted set command".into())),
    };
