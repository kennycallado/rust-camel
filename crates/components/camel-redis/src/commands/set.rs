...
pub async fn dispatch(
    ...
    let result: serde_json::Value = match cmd {
        RedisCommand::Sadd => {
            ...
            let n: i64 = conn.sadd(&key, value.to_string()).await
        ...
        RedisCommand::Srem => {
            let key = require_key(exchange)?;
            let value = require_value(exchange)?;
            let n: i64 = conn.srem(&key, value.to_string()).await
        ...
        RedisCommand::Sismember => {
            let key = require_key(exchange)?;
            let value = require_value(exchange)?;
            let ok: bool = conn.sismember(&key, value.to_string()).await
        ...
        RedisCommand::Smove => {
            ...
            let value = require_value(exchange)?;
            let ok: bool = conn.smove(&key, dest, value.to_string()).await
        ...
        RedisCommand::Sinterstore => {
            ...
            let keys = get_str_vec_header(exchange, "CamelRedis.Keys")
                .ok_or_else(|| CamelError::ProcessorError("Missing CamelRedis.Keys".into()))?;
            let n: i64 = conn.sinterstore(dest, &keys).await
                .map_err(|e| CamelError::ProcessorError(format!("Redis SINTERSTORE failed: {e}")))?;
            serde_json::json!(n)
        }
        RedisCommand::Sunionstore => {
            ...
            let keys = get_str_vec_header(exchange, "CamelRedis.Keys")
                .ok_or_else(|| CamelError::ProcessorError("Missing CamelRedis.Keys".into()))?;
            let n: i64 = conn.sunionstore(dest, &keys).await
                .map_err(|e| CamelError::ProcessorError(format!("Redis SUNIONSTORE failed: {e}")))?;
            serde_json::json!(n)
        }
        RedisCommand::Sdiffstore => {
            ...
            let keys = get_str_vec_header(exchange, "CamelRedis.Keys")
                .ok_or_else(|| CamelError::ProcessorError("Missing CamelRedis.Keys".into()))?;
            let n: i64 = conn.sdiffstore(dest, &keys).await
        ...
        RedisCommand::Srandmember => {
            ...
            match count {
                Some(c) => {
                    let members: Vec<String> = conn.srandmember_multiple(&key, c as isize).await
                        .map_err(|e| CamelError::ProcessorError(format!("Redis SRANDMEMBER failed: {e}")))?;
                    serde_json::json!(members)
                }
                None => {
                    let member: Option<String> = conn.srandmember(&key).await
                        .map_err(|e| CamelError::ProcessorError(format!("Redis SRANDMEMBER failed: {e}")))?;
                    member.map(serde_json::Value::String).unwrap_or(serde_json::Value::Null)
                }
            }
        }
...
mod tests {
    ...
    fn test_sadd_has_key_and_value() {
        ...
        assert_eq!(crate::commands::require_value(&ex).unwrap(), serde_json::json!("member1"));
    }
