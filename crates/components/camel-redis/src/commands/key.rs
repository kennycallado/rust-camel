...
use super::{get_str_header, get_u64_header, get_str_vec_header, require_key};
...
pub async fn dispatch(
    ...
    let result: serde_json::Value = match cmd {
        RedisCommand::Exists => {
            ...
            let n: bool = conn.exists(&key).await
        ...
        RedisCommand::Del => {
            let keys = get_str_vec_header(exchange, "CamelRedis.Keys")
                ...
                .ok_or_else(|| CamelError::ProcessorError("Missing CamelRedis.Key or CamelRedis.Keys".into()))?;
        ...
        RedisCommand::Expire => {
            ...
            let ok: bool = conn.expire(&key, secs).await
        ...
        RedisCommand::Expireat => {
            ...
            let ok: bool = conn.expire_at(&key, ts).await
                .map_err(|e| CamelError::ProcessorError(format!("Redis EXPIREAT failed: {e}")))?;
            serde_json::json!(ok)
        }
        RedisCommand::Pexpire => {
            let key = require_key(exchange)?;
            let ms = get_u64_header(exchange, "CamelRedis.Timeout").unwrap_or(0) as i64;
            let ok: bool = conn.pexpire(&key, ms).await
                .map_err(|e| CamelError::ProcessorError(format!("Redis PEXPIRE failed: {e}")))?;
            serde_json::json!(ok)
        }
        RedisCommand::Pexpireat => {
            let key = require_key(exchange)?;
            let ts = get_u64_header(exchange, "CamelRedis.Timestamp").unwrap_or(0) as i64;
            let ok: bool = conn.pexpire_at(&key, ts).await
                .map_err(|e| CamelError::ProcessorError(format!("Redis PEXPIREAT failed: {e}")))?;
            serde_json::json!(ok)
        }
        RedisCommand::Ttl => {
            let key = require_key(exchange)?;
            let n: i64 = conn.ttl(&key).await
        ...
        RedisCommand::Rename => {
            ...
            conn.rename::<_, _, ()>(&key, dest).await
        ...
        RedisCommand::Renamenx => {
            ...
            let dest = get_str_header(exchange, "CamelRedis.Destination")
                .ok_or_else(|| CamelError::ProcessorError("Missing CamelRedis.Destination".into()))?;
            let ok: bool = conn.rename_nx(&key, dest).await
                .map_err(|e| CamelError::ProcessorError(format!("Redis RENAMENX failed: {e}")))?;
            serde_json::json!(ok)
        }
        RedisCommand::Type => {
            let key = require_key(exchange)?;
        ...
        RedisCommand::Persist => {
            let key = require_key(exchange)?;
            let ok: bool = conn.persist(&key).await
                .map_err(|e| CamelError::ProcessorError(format!("Redis PERSIST failed: {e}")))?;
            serde_json::json!(ok)
        }
        RedisCommand::Move => {
            let key = require_key(exchange)?;
            let db = exchange.input.header("CamelRedis.Db")
                .and_then(|v| v.as_u64()).unwrap_or(0) as i64;
            // MOVE is not directly exposed in AsyncCommands, use raw command
            let ok: i64 = redis::cmd("MOVE").arg(&key).arg(db).query_async(conn).await
        ...
        RedisCommand::Sort => {
            ...
            let vals: Vec<String> = redis::cmd("SORT").arg(&key).query_async(conn).await
