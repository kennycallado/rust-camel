...
- **Health check** (`fn ping`, health.rs:48): `get_multiplexed_async_connection()` wrapped in `connection_timeout`.
- **Producer** (`fn get_or_create_connection`, producer.rs:248): lazy `get_multiplexed_async_connection()` wrapped in `connection_timeout`.
- **Consumer PubSub** (`fn run_pubsub_consumer`, consumer.rs:257): `get_async_pubsub()` wrapped in `connection_timeout`.
- **Consumer Queue** (`fn run_queue_consumer`, consumer.rs:342): `get_multiplexed_async_connection()` wrapped in `connection_timeout`.

### Health check outer timeout (`fn new`, health.rs:91)

Derived from `connection_timeout_secs + 5` seconds. The outer timeout at `check()` (`fn check`, health.rs:108)
