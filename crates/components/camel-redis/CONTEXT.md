...
Since bd rc-hbde6 the sentinel surface trusts the same PEM: the `TopologyKind::Sentinel` arm (with the `tls` feature) calls `SentinelTopology::new_with_ca`, which feeds redis-rs's `SentinelClientBuilder` with the PEM as root trust for BOTH link surfaces — the sentinel discovery connections (whose `rediss://` node URLs carry `TcpTls` addresses) and the resolved master/replica connections (`set_client_to_redis_tls_mode(TlsMode::Secure)` + certificates). Without the `tls` feature the arm keeps the credential-embedding `SentinelTopology::new` path. Live proof: `crates/camel-test/tests/redis_sentinel_tls_test.rs`.
