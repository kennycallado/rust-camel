...
The system SHALL allow users to compose stale-on-error resilience by combining the existing
`CircuitBreaker` step (with its `fallback:` sub-pipeline) and the `cache_peek_stale` step.
No feature of the cache SHALL bake stale-on-error into the `CacheRepository` trait or its
backends. The composition SHALL be demonstrable end-to-end.

#### Scenario: circuitBreaker fallback serves cached stale entry on upstream failure

- **GIVEN** a route of the shape `circuitBreaker: { threshold: 1, steps: [ <upstream fetch> ],
  fallback: [ cache_peek_stale: { repository: persistent, key: "tile-xyz" } ] }` and a
  `"persistent"` repository holding a stale (past-expiry) entry under `"tile-xyz"`
- **WHEN** the upstream fetch fails so the circuit opens
