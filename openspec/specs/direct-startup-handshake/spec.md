...
The `DirectProducer::poll_ready()` SHALL retain its existing behavior: when the
registry contains `None` for the producer's name and `fail_if_no_consumers` is
not `Some(false)`, it SHALL return `Poll::Ready(Err(EndpointCreationFailed))`.
The startup handshake eliminates the auto-startup race window that previously
caused spurious `None` observations; this error path now fires only for
genuinely-misconfigured routes (no consumer exists at all).
