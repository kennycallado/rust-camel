...
The tag matrix SHALL build the three-flavor topology (14 legs):
slim on x86_64/aarch64 musl; regular on all 7 targets; full on gnu
×2 (aarch64 on the native `ubuntu-24.04-arm` runner), macOS ×2, and
Windows. Each leg SHALL select its closure exclusively via the
...
- **THEN** exactly 3 build legs run — x86_64-unknown-linux-gnu
  ...
  `dev-profile: false` (tag path) all 14 legs run
