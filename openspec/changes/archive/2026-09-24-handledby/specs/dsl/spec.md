...
Aligns the wildcard requirement with the clause-level `handled_by`
layout (e_opus ruling, bd rc-ntpof). Wildcard matching semantics are
unchanged; only the `handled_by` layout moves out of `retry:`.
