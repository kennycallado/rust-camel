...
The system SHALL enable the tracer pipeline when `[observability.prometheus]`
is enabled and `[observability.tracer].enabled` is not explicitly set. An
explicit `enabled` value in `[observability.tracer]` MUST take precedence
over this implication in both directions.
