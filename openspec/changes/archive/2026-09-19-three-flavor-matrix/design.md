...
## Decision 3 — Matrix topology (12 tag entries, 11 uploading)

POST-rc.2 OWNER RULING (2026-09-19): desktop platforms ship FULL only —
macOS/Windows regular legs deleted (regular is a server flavor; desktop
users want everything; dropping now and re-adding later is non-breaking
per the iteration policy). The darwin-x86_64 regular entry stays as a
compile-guard (upload: false) proving the regular closure builds on
macOS for cargo-install source users.

| leg | target | runner | flavor |
|---|---|---|---|
| 1–2 | x86_64/aarch64-unknown-linux-musl | ubuntu | slim |
| 3–4 | x86_64/aarch64-unknown-linux-gnu | ubuntu | regular |
| 5–6 | x86_64/aarch64-unknown-linux-musl | ubuntu | regular |
| 7 | x86_64-apple-darwin | macos-intel | regular (compile-guard, upload: false) |
| 8–9 | x86_64/aarch64-unknown-linux-gnu | ubuntu / ubuntu-24.04-arm (aarch64 native) | full |
| 10–11 | x86_64/aarch64-apple-darwin | macos-intel / macos-arm | full |
| 12 | x86_64-pc-windows-msvc | windows | full |
...
the reusable matrix (default `false` = full 12 entries). When true, the build
