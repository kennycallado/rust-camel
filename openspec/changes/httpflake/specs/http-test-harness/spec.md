# http-test-harness Delta

Capability: test-harness laws for `camel-component-http` lib tests that boot
a real consumer through the global `ServerRegistry`. Local to camel-http's
test module; it does not modify the cross-component
`staged-listener-binding` canon.
