...
| `suspend_route` / `resume_route` reales                              | AutoStart postmortem                   | Media-baja — implementación actual es alias de stop/start; suspensión real requiere buffer de in-flight messages |        |
| ControlBus: acción `stats`                                           | AutoStart Future Work                  | Baja — devolver métricas de ruta (exchanges procesados, uptime, etc.)          |        |
| Tests de integración: aislar paralelismo (serial_test o similar)     | AutoStart postmortem                   | Media-baja — `camel-test` tests se cuelgan con `cargo test` por defecto; workaround actual: `--test-threads=1` |        |
...
| camel-cli: package name reservation                                  | camel-cli plan                         | Baja — CLI mínima publicada en crates.io para reservar nombre; sin funcionalidad real aún |        |
...
| Simple Language v2: `${body.fieldName}` JSON path                    | Languages postmortem                   | Media-baja — actualmente `${body}` devuelve texto plano; acceso a campos JSON requiere serde_json Pointer |        |
...
| Hot-reload: ReloadCoordinator execution                              | ArcSwap plan + postmortem              | Media — coordinator solo computa diff; ejecución (swap_pipeline/stop/start) se agrega cuando camel-cli lo necesite |        |
...
| YAML DSL: más step types (filter, split, choice, etc.)               | ArcSwap plan note #3                   | Media-baja — parser intencionalmente minimal (solo to/set_header/log); extensible sin cambios arquitectónicos |        |
| YAML DSL: error handler config                                        | Resilience design Non-Goals            | Baja — circuit breaker config en YAML diferido a futuro                        |        |
