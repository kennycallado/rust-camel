...
| rc-audm.8 | RESOLVED by blessed trailing-window protocol (benchwarmup) — policy: `harness/CONTEXT.md` §2 | node-native http m2 STILL unconverged on current code (4/5 rounds MessageBoundUnconverged; 5/14 stable in sweep). Root cause: stability window hard-capped at first max_messages (warmup.rs:145 second_end=min(n,max_messages)); 1000-msg bound fires ~40x before 30s wall at http rates; protocol A has no adaptive-extension path. Fix landed: trailing-window time-based comparison, blessed as benchwarmup — Protocol A now collects to the wall-clock deadline and evaluates the latest complete window. Harness classifier matches the new outcome set: run.sh native-status grep + summarize.py fallback evidence rules accept ALL failed-stability reasons (MessageBoundUnconverged historical; TimeBoundUnconverged / InsufficientSamples future) as unconverged/attempted, so future unconverged cells keep native attempted status. Sealed record never re-run, never republished |
