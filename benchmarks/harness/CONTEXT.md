...
| **Marker** | The exact stdout string `BENCH_ROUTE_READY`, printed exactly once by every artifact when its route is live. The harness's clock stops on the marker. Cold-start fixtures print ONLY the marker — no self-timing (see §2). Tick-mode exception (bench-consol-tick): the warm tick scenarios (`t2-json`, `split-aggregate`, `t2-realistic-eip`) additionally append one `BENCH_LATENCY <id> <ns>` record to `$BENCH_LATENCY_FILE` per 10ms tick — measurement records for the harness's protocol-B parser, not a second marker and not a cold-start clock; M1 cold-start remains zero-self-timing. The rust-camel-cli tick bodies are frozen (rc-sgmk fixed): bodies build once per process and the digest logs once, so the cli tick window contains only exchange processing, like the other contenders — see RUNBOOK §4. |
| **Fixture** | The per-artifact source + build config that produces one artifact and emits the marker. E.g. `camel-standalone-dsl/App.java`, `rust-camel-lib` `src/scenarios/<scn>.rs` (consolidated crate, argv dispatch — moved from per-scenario `main.rs` by bench-consol-tick), `rust-camel-cli/Camel.toml` + `routes/startup-minimal.yaml`. |
...
4. [cross-ref] Confirmed: the lib fixture emits `.log("BENCH_ROUTE_READY", LogLevel::Info)` — was `rust-camel-lib/src/main.rs`, now `benchmarks/contenders/rust-camel-lib/src/scenarios/<scn>.rs` (all 7 modules; moved by bench-consol-tick); harness asserts `marker_count -ne 1` is a hard failure (`run.sh` measure_once). Marker contract is real. T2 marker is the same `BENCH_ROUTE_READY` token but with `body=pong-bench` suffix on the same line — the harness `grep -cF` counts the exact full string.
5. [v2] Each new term has a stable, citable definition: Substrate VM is the GraalVM native-image runtime (`spike-results.md` Spike 1B); native initialization is the build-time shift (v2 report "Native initialization caveat" section); fingerprint caching is `lib/native_cache.sh` `native_cache_plan` (rc-wdy13); Pair A predicate deviation is the spec §4.1 document. All four are recorded as glossary rows so v3+ contributors can find them without re-reading the spec/plan.
...
| **Single harness-side clock (no in-process timing)** | Two rejected alternatives bracket the truth: (1) timing `docker run` from *outside* over-counts (container overhead); (2) self-instrumenting from inside `main()` under-counts (skips JVM creation, bootstrap classloading, JIT init — all *before* user code, and all part of real cold-start). A single wall-clock captured immediately before `exec` and stopped at the marker captures full bootstrap with no double-clock fragility. Fixtures therefore carry **zero self-timing** — they only print the marker. Tick-mode exception (bench-consol-tick): the three warm tick scenarios (`t2-json`, `split-aggregate`, `t2-realistic-eip`) keep looping after the marker on a 10ms timer (first fire immediate; `delay=0` on JVM Camel) and append one `BENCH_LATENCY` record per tick to `$BENCH_LATENCY_FILE` — the fixture times its own route work per tick because the harness cannot see inside the route (that per-tick service time IS the M2 signal), but the cold-start clock stays harness-side and the M1 path remains strictly zero-self-timing. |
...
| **v3: Camel Quarkus JVM variants dropped from measurement** | `camel-quarkus-dsl` and `camel-quarkus-yaml` (JVM mode) duplicate the "Camel bootstrap on HotSpot" signal already covered by `camel-standalone-dsl` / `camel-standalone-yaml` — the production Camel classic baseline. v1/v2 reports retain the JVM numbers as native-vs-JVM mode-delta reference (see COVERAGE.md "Removed" section); v3+ no longer re-measures them. Native variants (`-native` suffix) remain because they represent Apache's marketed answer to startup cost — the comparison `rust-camel-lib vs camel-quarkus-dsl-native vs camel-standalone-dsl` is the ICP-qualifying triple. Matrix reduction: 32 → 26 cells. |
| **v3: Bridge scenarios stay at 4 artifacts (asymmetric)** | T4a/T4b measure the bridge subprocess tax that rust-camel pays (no native Rust XSLT/XSD engines; must delegate to Java subprocess via gRPC mTLS). The tax is **authoring-format-invariant**: the gRPC subprocess + byte-pinned XML payload are identical regardless of whether the route was authored in Java DSL or YAML. T1/T2 already isolate the YAML-parse overhead (~5ms). Adding `camel-standalone-yaml` + `camel-quarkus-yaml-native` for T4a/T4b would re-measure a known-invariant quantity — recorded as `won't-measure` in COVERAGE.md per e_opus round-5 verdict. Bridge matrix: 4 cells (camel-standalone-dsl + camel-quarkus-dsl-native + rust-camel-lib + rust-camel-cli). |
| **Roster authored in three places (intentional, drift-guarded)** | The 53-cell roster arithmetic (5 full scenarios × 8 contenders + 2 bridge scenarios × 6 + 1 reference contender) is hand-maintained in THREE files, each a different projection: `run.sh` (PAIR_A/PAIR_B_CONTENDERS affinity pairing, SCENARIO_ARTIFACT_SET asymmetry, SCENARIO_M2_PROTOCOL warm-applicability, FAMILY_COMPLETENESS + the bridge resolver's add_cell registrations — interleaved with argv/marker/defer wiring), `summarize.py` (FULL_CONTENDERS / BRIDGE_CONTENDERS / WARM_APPLICABLE / BRIDGE_SCENARIOS — flat tuples feeding expected_roster and the publish gate), and `checks/warm-24.py` (tick-scenario + contender subset for the warm exit gate). The reference contender (`axum-bare`, http-server-only cell, rc-u034) adds a FOURTH projection: `run.sh` `REFERENCE_CONTENDERS` ↔ `summarize.py` `HTTP_REFERENCE_CONTENDERS`, guarded by the same drift test. `checks/warm-24.py` stays at 24 — the reference contender is not a tick contender. Single-sourcing from one roster.json was evaluated (bd rc-2k33) and rejected: run.sh's projection is order-sensitive (pairing drives affinity) and entangled with registration logic a data file cannot express; runtime parsing would also weaken the drift guard from value-comparison to mechanism-trust and add a subprocess dependency to harness startup. The contract is instead enforced by `test_summarize.py::test_roster_mirror_no_drift`, which greps the run.sh SOURCE (never executes it) and fails on any drift among the projections. When adding a contender or scenario, update all three files (and the reference maps when adding a reference contender). |
| **Protocol A warmup: trailing-window, wall-clock-terminated (benchwarmup)** | Protocol A warmup collects requests until its wall-clock deadline; `max_messages` is the size of the trailing comparison-window, NOT a termination cap. At the deadline the criterion evaluates the latest complete window exactly once: halves within 10% → `Stable`; drifted → `TimeBoundUnconverged`; fewer than `max_messages` samples → `InsufficientSamples`. In-flight requests and body drains are deadline-guarded; late work is discarded and never extends warmup. `MessageBoundUnconverged` is retained only for public/schema compatibility — Protocol A never emits it. Protocol B (per-tick `BENCH_LATENCY` service-time) is unchanged. Sealing rule (e_opus ruling 2026-09-25): a sealed record's `run.json` and its `index.json` entry — e.g. `benchmarks/records/20260903T084658Z` — are never modified; `summary.md` is a derived view regenerated by the current renderer, which computes missing flags from the record's own data (unknown is never rendered as not-flagged). Live-defect disposition (rc-audm.8): `benchmarks/docs-investigation-strategy.md` §8. |
...
| Camel Quarkus | `camel-quarkus-dsl-native/` — hardcoded `BenchRoute.java`, native image (v3: JVM variant dropped — redundant with `camel-standalone-dsl`) | `camel-quarkus-yaml-native/` — YAML route + native image (v3: JVM variant dropped) |
...
4. [cross-ref] Confirmed: the lib fixture imports only `TimerComponent` and builds the route programmatically with `struct RouteBuilder` + `.log(...)` — no YAML, no serde_yaml path (was `main.rs:22,40,49` pre-consolidation; now the per-scenario modules under `benchmarks/contenders/rust-camel-lib/src/scenarios/`). Pair A assignment is real, not aspirational.
...
| **Protocol-A common-rate inertness of reference cells** | `calibrate_protocol_a` derives `PROTOCOL_A_COMMON_RATE` as the MIN of per-cell max-sustainable rates (each measured independently against the fixed absolute p99 ≤ 100 ms bar), so a reference contender faster than the slowest cell (e.g. `axum-bare`) cannot shift the common rate or any other cell's m2 measurement — a faster cell is inert by MIN semantics. |
