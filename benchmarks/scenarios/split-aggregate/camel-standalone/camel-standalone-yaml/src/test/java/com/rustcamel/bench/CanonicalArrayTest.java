...
class CanonicalArrayTest {
    ...
    void canonicalArrayLength() {
        assertEquals(591, AppYaml.canonicalArray().length());
        assertEquals(AppYaml.CANONICAL_ARRAY_BYTES, AppYaml.canonicalArray().length());
    }
    ...
    void goldenDigest() {
        assertEquals(
                ...
                AppYaml.sha256Hex(AppYaml.canonicalArray()));
    }
}
