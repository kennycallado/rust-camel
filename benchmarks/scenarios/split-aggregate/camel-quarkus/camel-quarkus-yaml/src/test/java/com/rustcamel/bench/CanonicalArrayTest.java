...
class CanonicalArrayTest {
    ...
    void canonicalArrayLength() {
        assertEquals(591, BenchBeans.canonicalArray().length());
        assertEquals(BenchBeans.CANONICAL_ARRAY_BYTES, BenchBeans.canonicalArray().length());
    }
    ...
    void goldenDigest() {
        assertEquals(
                ...
                BenchBeans.sha256Hex(BenchBeans.canonicalArray()));
    }
}
