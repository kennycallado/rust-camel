...
cleanup_cell_artifacts() {
    ...
    pkill -9 -f "$NODE_NATIVE_DIR/xslt-bridge.mjs" 2>/dev/null || true
    pkill -9 -f "$NODE_FASTIFY_DIR/xslt-bridge.mjs" 2>/dev/null || true
...
# (Saxon-JS ≠ Saxon-HE serializers; see contenders/node/README.md).
...
launch_node_native() {
    ...
    "$NODE_BIN" "$NODE_NATIVE_DIR/xslt-bridge.mjs"
}
...
launch_node_fastify() {
    ...
    "$NODE_BIN" "$NODE_FASTIFY_DIR/xslt-bridge.mjs"
}
...
if [[ -x "$NODE_BIN" && -d "$NODE_CONTENDER_DIR/node_modules/saxon-js" && -d "$NODE_CONTENDER_DIR/node_modules/xslt3" && -f "$NODE_NATIVE_DIR/xslt-bridge.mjs" ]]; then
...
if [[ -x "$NODE_BIN" && -d "$NODE_CONTENDER_DIR/node_modules/fastify" && -d "$NODE_CONTENDER_DIR/node_modules/saxon-js" && -d "$NODE_CONTENDER_DIR/node_modules/xslt3" && -f "$NODE_FASTIFY_DIR/xslt-bridge.mjs" ]]; then
