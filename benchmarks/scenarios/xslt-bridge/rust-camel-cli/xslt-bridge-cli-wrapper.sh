#!/usr/bin/env bash
# T4a XSLT bridge fixture — rust-camel-cli (Pair B partner, bd Task 4).
#
# Wrapper that:
...
#   xslt-bridge-cli-wrapper.sh \
#     --camel-bin <path/to/camel> \
#     --config <fixture>/Camel.toml \
#     --routes <fixture>/routes/xslt-bench.yaml \
...
bridge_wrapper=""
bridge_pid_file=""
latency_file="/tmp/v3-protocol-b-t4a-rust-camel-cli.log"

usage() {
    cat <<'EOF' >&2
usage: xslt-bridge-cli-wrapper.sh
  --camel-bin <path/to/camel>
  --config <fixture>/Camel.toml
  --routes <fixture>/routes/xslt-bench.yaml
  --bridge-binary <worktree>/bridges/xml/build/native/xml-bridge
...
work_dir="$(mktemp -d -t t4a-cli-wrapper-XXXXXX)"
