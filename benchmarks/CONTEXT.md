...
4. [cross-ref] Confirmed: `rust-camel-lib/src/main.rs` imports only `TimerComponent` and builds the route programmatically with `struct RouteBuilder` + `.log(...)` — no YAML, no serde_yaml path (`main.rs:22,40,49`). Pair A assignment is real, not aspirational.
