...
Per [ADR-0025](../adr/0025-outcome-aware-structural-eips.md), the recipient list is an outcome-aware structural EIP. A branch that returns `PipelineOutcome::Stopped` records a partial failure. The aggregation skips the stopped branch, and the route error handler sees the partial outcome through the same boundary that step errors flow through.
