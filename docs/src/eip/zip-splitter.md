...
The ZIP Splitter is format-specific. It knows how to parse the ZIP archive structure. For TAR and TAR.GZ archives, the [TAR Splitter](tar-splitter.md) applies the same entry-per-exchange model through its own TAR parser; there is no generic archive-splitting abstraction. The [Streaming Splitter](streaming-splitter.md) is the alternative for byte streams. It splits any byte stream through a codec that the content type selects. Use the ZIP Splitter when the input is a ZIP archive and you need per-entry headers. Use the Streaming Splitter for NDJSON, log lines, or raw byte chunks.
