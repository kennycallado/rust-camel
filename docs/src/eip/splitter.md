...
**Fragment typing.** When the split expression evaluates to an array, each element's type decides the fragment body. String elements become `Body::Text` fragments, so `${body}` renders the raw string with no JSON quotes, and a string types the same way whether the expression matched one node or many. Number, boolean, object, nested-array, and null elements become `Body::Json` fragments; their JSON text form carries no quotes, so `${body}` output is unchanged for them. An empty-string element renders as an empty string, not `""`. The programmatic `split_body_json_array()` splitter follows the same element-driven rule. `CollectAll` aggregation output is the same either way, because both fragment typings aggregate to the same JSON string value.
