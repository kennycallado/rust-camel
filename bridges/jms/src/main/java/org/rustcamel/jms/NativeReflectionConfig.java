 ...
 *
 * Uses {@code classNames} (String-based) instead of {@code targets} (class-literal)
 * to avoid build-time class loading which can pull in classes with static Random
 * fields (e.g. RandomUtil) that cause "Random in image heap" build failures.
 *
 * The classes listed here are loaded via Class.forName() at runtime by ActiveMQ
 * Classic (OpenWire protocol), Artemis JMS client, and connection pooling libraries.
 */
