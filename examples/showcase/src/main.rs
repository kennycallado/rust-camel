...
async fn main() -> Result<(), CamelError> {
    ...
    // Route 7 (Filter Pipeline Stop): Demonstrates that Filter stops pipeline when predicate=false
    // Timer generates alternating "important" and "normal" messages
    // Filter only passes "important" messages
    // log:filter-inside executes ONLY for "important" messages (inside filter)
    // log:filter-after should ONLY show "important" messages (after Filter)
    // "normal" messages are FILTERED OUT and never reach log:filter-after
    ...
    let route7 = RouteBuilder::from("timer:filter-demo?period=2000&repeatCount=6")
        .process(move |mut exchange| {
            ...
            exchange.input.body = Body::Text(format!("[{}] {} message", msg_type, msg_type));
            exchange
                .input
        ...
        .end_filter()
        .to("log:filter-after?showBody=true&showHeaders=true")
        .build()?;
    ...
    println!("  - timer:filter-demo (2s) -> filter (stops pipeline for 'normal')");
    println!();
