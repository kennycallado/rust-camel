...
- [Config](./crates/camel-config/CONTEXT.md) — platform configuration loading and resolution: `CamelConfig` deserialization, unified `${env:}` placeholder interpolation (tree walk via camel-dsl `interpolate_env`), component discovery, and hot-reload wiring
- [Components](./crates/components/CONTEXT.md) — inbound/outbound adapters (timer, HTTP, Kafka, file, etc.) that feed Exchanges into Routes and send them to external systems
  ...
  - [SurrealDB Component](./crates/components/camel-component-surrealdb/CONTEXT.md) — multi-model database Component for 12 SurrealQL document, graph, vector, function, and live-query operations through a named datasource
  - [MCP Component](./crates/components/camel-component-mcp/CONTEXT.md) — first-class MCP (Model Context Protocol) Component (scheme `mcp:`): server Consumer role exposing tools/resources over one shared Streamable-HTTP listener per bind, plus client Producer role dispatching `mcp:call` / `mcp:read` to remote servers; protocol baseline `2026-07-28` stateless (ADR-0060)
...
- [Services](./crates/services/CONTEXT.md) — cross-cutting infrastructure services: observability (OTel, Prometheus), auth/security, and platform integration (Kubernetes)
   ...
   - [Redis Repository Service](./crates/services/camel-redis-repo/CONTEXT.md) — Redis-backed `CacheRepository` and `IdempotentRepository` implementations; one multiplexed connection per repository, prefix-scoped key isolation, fail-closed config selection (ADR-0063)
...
- **Services → Runtime (repository registration)**: repository service crates implement `camel-api` repository traits (`IdempotentRepository`, `CacheRepository`) and register into `CamelContext` by name at context build time; EIP steps resolve them by name during Exchange processing (ADR-0063)
...
- [0035](./docs/adr/0035-leader-epoch-fencing-token.md) — Every `master:` delegate envelope carries the `x-camel-leader-epoch` monotonic fencing token, captured when its bridge starts; downstream sinks should reject older epochs for split-brain safety
...
- [0020](./docs/adr/0020-llm-component-provider-adapter-boundary.md) — LLM component isolates siumai SDK behind a project-owned LlmProvider trait; production siumai imports are confined to `provider/siumai_adapter.rs`
...
- [0023](./docs/adr/0023-idempotent-repository-trait.md) — `IdempotentRepository` trait lives in `camel-api` (key-only, `Result`-returning) so any crate can implement it; backends propagate transient read failures. Stores keys, not full messages (Claim Check is ADR-0028) _(amended by 0063)_
...
- [0060](./docs/adr/0060-mcp-first-class-component.md) — MCP as a first-class Server+Host Component (scheme `mcp:`): route-owned tool dispatch (no auto-loop), DSL-lowered catalog, rmcp confined to `src/adapter/` (ADR-0020), Streamable HTTP only, protocol baseline `2026-07-28` stateless; exclusions: Prompts, stdio, sessions, legacy transports, subscriptions
- [0061](./docs/adr/0061-unified-transport-auth.md) — Unified transport authentication kernel: five kernel transports (http, ws, grpc, mcp, wasm source) extract credentials and render denial; the wasm source authenticates at the host edge (Gen B, like ws/grpc), the carrier threads through `SourceHostState` to Exchange assembly; `kernel_authenticate` mints the unforgeable `AuthenticatedPrincipal` (same-crate-only construction, zero public or test-only constructors), per-route `RouteSecurityPlan` compiles access mode + provider; `Public` default only for undeclared routes, gated by per-bind `allow_public_exposure` with ADR-0052 rule-3 permanent warn; `trust_upstream_principal` removed (pre-1.0 breaking, stale configs fail at load); ADR-0060 Rule 3 amended, Rule 8 superseded _(amends 0060; supersedes 0059 in part)_
- [0062](./docs/adr/0062-reserved-test-suffix-and-placement-contract.md) — Reserved test suffix and placement contract: `.test.yaml`/`.test.yml` names a `camel test` document, not a route; discovery skips it under wildcard globs and errors on literal naming (explicit-gate idiom); the rule lives only in `is_test_document` in camel-dsl discovery; colocation is the blessed placement, a separate directory is first-class via `routeFilesFromRoot` anchored at the nearest ancestor `Camel.toml`; explicit `*.test.yaml` glob override removed (pre-1.0 breaking)
- [0063](./docs/adr/0063-redis-repository-service.md) — Redis repository service: `camel-redis-repo` in `crates/services/` implements both `IdempotentRepository` and `CacheRepository`; connection seam is three additive widenings in `camel-component-redis`; one executor per repository (no registry); Sentinel always compiled in the service crate with explicit-config selection; `SET NX` never re-issued after a transport error (C1); prefix-scoped SCAN+UNLINK clear with charset guards; transient errors map to `Io`, setup errors to `Config` _(amends 0023, 0056)_
...
- **Cache EIP** — The `cache`/`cache_invalidate`/`cache_peek_stale`/`cache_clear`/`cache_stats` DSL steps, compiled to runtime `CacheService`/`CacheInvalidateService`/`CachePeekStaleService`/`CacheClearService`/`CacheStatsService` (`OutcomeSegment` impls). `cache_invalidate` takes either `key` (exact) or `key_prefix` (namespace purge that sets the `CamelCacheInvalidatedCount` property and fails closed on backends without key iteration); `cache` takes `coalesce_misses` (singleflight: concurrent misses on one key fetch once). Supported on both the DSL path and the canonical `RegisterRoute` control-plane converter. Disambiguation: UNRELATED to `stream_cache` (OOM protection, see `StreamCacheService`). See ADR-0056.
- **CacheEntry** — The value stored in a `CacheRepository`: materialized bytes as `Vec<u8>` + content-type discriminant + in-band expiry timestamp. NOT `Body` (which is not `Serialize` and includes the un-cacheable `Stream` variant). See ADR-0056.
- **CacheRepository** — Pluggable TTL cache port in camel-api. Stores `CacheEntry { bytes: Vec<u8>, content_type, expires_at }`. `invalidate_prefix` is a default async method returning `Err` naming the backend (non-iterating backends fail closed without implementing it); `RedbCacheRepository` overrides it for namespace purges. `stats` is an async default method (pre-1.0 signature correction, bd rc-22wj) — redb computes its `bytes: Some(payload-sum)` inside `spawn_blocking`. Distinct from `IdempotentRepository` (key-only, no time) and `ClaimCheckRepository` (payload-owning, no expiry). See ADR-0056. Backends: memory, redb, and redis (`RedisCacheRepository` in the `camel-redis-repo` repository service crate, ADR-0063, which overrides `invalidate_prefix` with the same prefix-scoped SCAN+UNLINK machinery as its `clear()`).
- **Redis repository backend** — The Redis implementations of the `camel-api` repository traits: `RedisIdempotentRepository` and `RedisCacheRepository`, shipped in the `camel-redis-repo` repository service crate. Keys are namespaced `{prefix}:{repo}:{key}`, so `clear()` and `invalidate_prefix` stay scoped to one repository namespace (SCAN+UNLINK, never FLUSH). Idempotent `add` issues one `SET NX` and never re-issues it after a transport error (Contract C1). Established by ADR-0063. (crates/services)
- **repository service crate** — A crate in `crates/services/` that implements a `camel-api` repository trait and registers into `CamelContext` by name at context build time. Its reads and writes run during Exchange processing. It is not a Component, and it is not necessarily a `Lifecycle` implementation. Established by ADR-0063. (crates/services)
...
- **Vertical slice (camel-core)** — Unit of decomposition for camel-core: each bounded context is a self-contained slice with its own internal hexagonal layout (`domain`/`application`/`ports`/`adapters`), NOT a horizontal technical layer shared across contexts. Examples today: `lifecycle/`, `hot_reload/`. Flat root modules are slices awaiting internal organization. Established by ADR-0045.
- **Bounded context (camel-core)** — A behavioral area that owns its own domain vocabulary and is the unit at which architectural decisions (notably CQRS flavor) are made: route lifecycle control plane, data plane, hot-reload, metrics, audit. CQRS is decided per bounded context, never crate-wide. Established by ADR-0045.
- **Synchronous-projection CQRS** — CQRS variant where the read-side projection is updated within the same optimistic-versioned UnitOfWork as the command, giving strong read-model freshness with no projection lag. Distinct from eventual-consistency CQRS; "strong consistency" here means read-model freshness, not CAP linearizability. Used by the route lifecycle control plane. Established by ADR-0002 / ADR-0045.
- **Module-discipline ceiling** — camel-core's declared 1.0 ceiling: the Clean Architecture rings are enforced by module paths + boundary tests, not by compiler-enforced crate isolation, because camel-core deliberately stays one crate. A crate split remains a post-1.0 option if module discipline proves insufficient. Established by ADR-0045.
- **StopSegment**:
...
- **ConsumerStopping** — `CamelError` variant for producer shutdown signals. Raised in `call()` when the producer's semaphore is closing (moved from `poll_ready` by the fix-otel-direct-deadlock change). Distinct from Stop EIP — it indicates the producer's semaphore/channel is closing and the call cannot proceed. Used by JMS/OpenSearch producers. Established by ADR-0024. (camel-api + camel-component-jms + camel-component-opensearch)
...
- **CredentialSource** — The route-declared credential extraction sources for a `security_policy` block: `AuthorizationHeader`, `QueryParam { param }`, `Cookie { name }`, and `Header { name }` (an API key in a named custom header). A route lists them in `credential_sources`; extraction tries them in declared order and the first match wins. Every extracted value flows through the same constant-time store lookup. On component-owned paths (WS, wasm source) the component authenticates the token itself at the transport edge and passes the resulting principal to the policy. The `wasm:` source permits `AuthorizationHeader`, `Header`, `Cookie`, and `QueryParam`; it authenticates at the host edge (Gen B, like ws/grpc) and threads the principal to Exchange assembly. The removed `trust_upstream_principal` flag no longer exists and exchange-property principal evidence never authorizes. Authority: ADR-0059 (divergence + extraction), ADR-0051 (redaction), ADR-0061 (kernel). (camel-api + camel-auth + camel-core + camel-dsl + components/camel-http + components/camel-ws + components/camel-component-wasm)
- **MCP Server Consumer** — inbound MCP server role: exposes tools and resources through one shared Streamable-HTTP listener per bind; a tool/resource consumer route serves `tools/call` / `resources/read` through the per-listener registries. Established by ADR-0060. (camel-component-mcp)
- **MCP Client Producer** — outbound MCP client role: `mcp:call` / `mcp:read` dispatch exactly one JSON-RPC request per Exchange to a remote server through `McpServerMap`; a dispatch target, never a decider (no auto-loop). Established by ADR-0060. (camel-component-mcp)
- **Tool Registry** — per-listener `name → route` map of registered MCP tools backing `tools/list` and `tools/call`; duplicate names and cap breaches are rejected at consumer start. Established by ADR-0060. (camel-component-mcp)
- **Resource Registry** — per-listener `uri → route` map backing `resources/list` and `resources/read`; duplicate URIs and cap breaches are rejected at consumer start. Established by ADR-0060. (camel-component-mcp)
- **Shared Listener** — one Streamable-HTTP listener per bind serving all tool/resource routes on that server; a later consumer with conflicting config (`allowed_hosts`, TLS, catalog caps) is rejected fail-closed. Established by ADR-0060. (camel-component-mcp)
- **Reserved test suffix** — `.test.yaml`/`.test.yml` names a camel test document, not a route. Route discovery skips such files under wildcard globs. Naming one literally in a route pattern fails with the reserved-suffix error. Authority: ADR-0063. (camel-dsl + camel-cli)
- **Route/test placement** — colocation (`routes/foo.yaml` + `routes/foo.test.yaml` sidecar) is the blessed default. A separate directory is first-class via `routeFilesFromRoot`, anchored at the nearest ancestor `Camel.toml`. Authority: ADR-0063. (camel-cli)
