...
- **CacheRepository** — Pluggable TTL cache port in camel-api. Stores `CacheEntry { bytes: Vec<u8>, content_type, expires_at }`. `invalidate_prefix` is a default async method returning `Err` naming the backend (non-iterating backends fail closed without implementing it); `RedbCacheRepository` overrides it for namespace purges. Distinct from `IdempotentRepository` (key-only, no time) and `ClaimCheckRepository` (payload-owning, no expiry). See ADR-0056.
