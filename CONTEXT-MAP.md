...
- **Reserved test suffix** — `.test.yaml`/`.test.yml` names a camel test document, not a route. Route discovery skips such files under wildcard globs. Naming one literally in a route pattern fails with the reserved-suffix error. Authority: ADR-0062. (camel-dsl + camel-cli)
- **Route/test placement** — colocation (`routes/foo.yaml` + `routes/foo.test.yaml` sidecar) is the blessed default. A separate directory is first-class via `routeFilesFromRoot`, anchored at the nearest ancestor `Camel.toml`. Authority: ADR-0062. (camel-cli)
