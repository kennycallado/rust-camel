...
- **CredentialSource** — The route-declared credential extraction sources for a `security_policy` block: `AuthorizationHeader`, `QueryParam { param }`, `Cookie { name }`, and `Header { name }` (an API key in a named custom header). A route lists them in `credential_sources`; extraction tries them in declared order and the first match wins. Every extracted value flows through the same constant-time store lookup. On component-owned paths (WS), `trust_upstream_principal: true` means "accept the principal the component authenticated"; the spoof caveat applies only to the layer path. Authority: ADR-0059 (divergence + extraction), ADR-0051 (redaction). (camel-api + camel-auth + camel-core + camel-dsl + components/camel-http + components/camel-ws)
